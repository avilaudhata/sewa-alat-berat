...
function connect()
{
    return mysqli_connect("localhost", "root", "", "gerigihijau");
}
...
function addEbook($ebook)
{
    ...
    mysqli_query($conn, "INSERT INTO produk (added_by, title, Fungsi, category, type, link, cover) VALUES ('$username', '$title', '$Fungsi', '$category', '$type', '$link', '$cover')");
...
function getNewId()
{
    $cover = query("SELECT cover FROM produk ORDER BY id DESC", true);
...
function deleteEbook($ebook)
{
    ...
    $cover = query("SELECT cover FROM produk WHERE id = $id")["cover"];
...
function updateEbook($ebook)
{
    ...
    if (empty($id) || empty($title) || empty($Fungsi) || empty($category) || empty($type) || empty($link) || empty($status) || empty($oldCover)) {
        return false;
    }
    ...
    mysqli_query($conn, "UPDATE produk SET title = '$title', Fungsi= '$Fungsi', category = '$category', type = '$type', link = '$link', status = '$status', cover = '$cover' WHERE id = $id");
...
function searchEbook($keyword, $index = null, $ebookPerPage = null)
{
    if (isset($index, $ebookPerPage)) {
        return query("SELECT * FROM produk WHERE title LIKE '%$keyword%' OR FungsiLIKE '%$keyword%' OR category LIKE '%$keyword%' OR type LIKE '%$keyword%' OR status LIKE '%$keyword%' ORDER BY id DESC LIMIT $index, $ebookPerPage", true);
    }

    return query("SELECT * FROM produk WHERE title LIKE '%$keyword%' OR FungsiLIKE '%$keyword%' OR category LIKE '%$keyword%' OR type LIKE '%$keyword%' OR status LIKE '%$keyword%'", true);
}
